...
Covers: create, list, update (including in_review status), delete,
        comments, filtering, authorization.
"""
import uuid
...
class TestListAnnotations:
    async def test_list_returns_annotations(self, client: AsyncClient, unique_email: str):
        _, model_id = await _setup_user_project_model(client, unique_email)
    ...
    async def test_list_filter_by_status_open(self, client: AsyncClient, unique_email: str):
        _, model_id = await _setup_user_project_model(client, unique_email)
        ...
        # Resolve it
        with patch("app.services.annotations.publish_model_event", new_callable=AsyncMock):
            await client.patch(f"/v1/annotations/{annotation_id}", json={"status": "resolved"})
    ...
    async def test_list_filter_by_status_resolved(self, client: AsyncClient, unique_email: str):
        _, model_id = await _setup_user_project_model(client, unique_email)
...
class TestUpdateAnnotation:
    ...
    async def test_update_status_to_in_review(self, client: AsyncClient, unique_email: str):
        _, model_id = await _setup_user_project_model(client, unique_email)
        ann_id = await self._create_annotation(client, model_id)
...
class TestAnnotationComments:
    ...
    async def test_list_comments(self, client: AsyncClient, unique_email: str):
        _, model_id = await _setup_user_project_model(client, unique_email)
        ann_id = await self._create_annotation(client, model_id)

        await client.post(
            f"/v1/annotations/{ann_id}/comments", json={"body": "First comment"}
        )
        await client.post(
            f"/v1/annotations/{ann_id}/comments", json={"body": "Second comment"}
        )

        resp = await client.get(f"/v1/annotations/{ann_id}/comments")
        assert resp.status_code == 200
